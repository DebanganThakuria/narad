...
func TestDecodeHandleRejectsZeroNonce(t *testing.T) {
	t.Parallel()
	if _, err := DecodeHandle(EncodeHandle(Handle{Partition: 0, Offset: 1, Nonce: 0})); !errors.Is(err, ErrHandleMalformed) {
		t.Fatalf("DecodeHandle() error = %v, want %v", err, ErrHandleMalformed)
	}
}
...
func TestDecodeHandleRejectsNegativePartition(t *testing.T) {
	t.Parallel()
	if _, err := DecodeHandle(EncodeHandle(Handle{Partition: -1, Offset: 1, Nonce: 1})); !errors.Is(err, ErrHandleMalformed) {
		t.Fatalf("DecodeHandle() error = %v, want %v", err, ErrHandleMalformed)
	}
}
