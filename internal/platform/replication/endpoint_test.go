...
func TestStreamEndpoint(t *testing.T) {
	cases := []struct {
		...
		{name: "host port", addr: "127.0.0.1:7942", want: "http://127.0.0.1:7942/internal/v1/replicate/stream"},
		{name: "http scheme", addr: "http://127.0.0.1:7942", want: "http://127.0.0.1:7942/internal/v1/replicate/stream"},
		{name: "https scheme", addr: "https://node.example", want: "https://node.example/internal/v1/replicate/stream"},
		{name: "trailing slash", addr: "http://node.example/", want: "http://node.example/internal/v1/replicate/stream"},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := streamEndpoint(tc.addr); got != tc.want {
				t.Fatalf("streamEndpoint(%q) = %q, want %q", tc.addr, got, tc.want)
			}
		})
	}
}
