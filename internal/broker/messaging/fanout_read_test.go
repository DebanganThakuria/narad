...
func TestReadFanoutSlabReturnsKeyedRecords(t *testing.T) {
	...
	slab, err := e.ReadFanoutSlab(context.Background(), "parent", 1, 0, 100, 1<<20, 0)
	if err != nil {
	...
	if slab.DroppedBehind != 0 || slab.SkippedCorrupt != 0 {
		t.Fatalf("slab loss counters = %d/%d, want 0/0", slab.DroppedBehind, slab.SkippedCorrupt)
	}
...
func TestReadFanoutSlabTailInitAndCaps(t *testing.T) {
	...
	slab, err := e.ReadFanoutSlab(context.Background(), "parent", 0, topic.FanoutTailOffset, 100, 1<<20, 0)
	if err != nil {
	...
	slab, err = e.ReadFanoutSlab(context.Background(), "parent", 0, 0, 4, 1<<20, 0)
	if err != nil {
	...
	slab, err = e.ReadFanoutSlab(context.Background(), "parent", 0, 0, 100, 1, 0)
	if err != nil {
	...
	slab, err = e.ReadFanoutSlab(context.Background(), "parent", 0, 10, 100, 1<<20, 0)
	if err != nil {
...
func TestReadFanoutSlabLongPollWakesOnCommit(t *testing.T) {
	...
	go func() {
		slab, err := e.ReadFanoutSlab(context.Background(), "parent", 0, 1, 100, 1<<20, 5*time.Second)
		if err != nil {
...
func TestReadFanoutSlabValidation(t *testing.T) {
	...
	if _, err := e.ReadFanoutSlab(context.Background(), "ghost", 0, 0, 10, 1024, 0); !errors.Is(err, ErrTopicNotFound) {
		t.Fatalf("missing topic error = %v, want %v", err, ErrTopicNotFound)
	}
	if _, err := e.ReadFanoutSlab(context.Background(), "parent", 99, 0, 10, 1024, 0); !errors.Is(err, ErrInvalid) {
...
func (f *fakeFanoutLog) ReadKeyed(offset int64) (string, []byte, error) {
	if offset < f.oldest || offset >= f.hwm {
		return "", nil, storage.ErrOffsetNotFound
	}
	if f.corrupt[offset] {
		return "", nil, fmt.Errorf("%w: synthetic", errs.ErrCorruptRecord)
	}
	return fmt.Sprintf("k%d", offset), fmt.Appendf(nil, `{"o":%d}`, offset), nil
}
...
func TestReadFanoutSlabOnceDropBehindAndCorruptSkip(t *testing.T) {
	...
	slab, err := e.readFanoutSlabOnce(log, 3, 100, 1<<20)
	if err != nil {
	...
	slab, err = e.readFanoutSlabOnce(log, 0, 100, 1<<20)
	if err != nil {
