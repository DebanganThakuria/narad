...
// parent's is rejected (errs.ErrFanoutSchemaMismatch).
func (m *Manager) AttachChild(ctx context.Context, parent, child string) error {
	...
	if err := m.metastore.AttachChild(ctx, parent, child); err != nil {
		if errors.Is(err, errs.ErrNotFound) {
			return fmt.Errorf("%w: %v", ErrNotFound, err)
		}
		return err
	}
	m.logger.Info("fan-out child attached", "parent", parent, "child", child)
	return nil
}
