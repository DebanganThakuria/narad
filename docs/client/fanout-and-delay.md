...
- You need manage rights (ownership or `admin`) on **both** topics to attach: the child's [schema](schemas.md#fan-out-children) history is replaced by the parent's and the parent's records start flowing into it, so owning the parent alone does not let you claim someone else's topic. Detaching needs manage rights on either side. Listing children needs any grant on the parent.
