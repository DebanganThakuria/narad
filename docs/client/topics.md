...
| `schema` | Optional JSON Schema; payloads are validated on produce | none | see [Schemas](schemas.md); rejected payloads get `400` |
...
The single-topic response includes `partition_stats` (per-partition oldest offset, next offset, and segment counts, handy for eyeballing backlog and growth) and, when the topic has a schema, `schema_version` and the current `schema` document. `GET /v1/topics/orders/schema` returns every version; see [Schemas](schemas.md).
...
| `schema` | yes | registers a new schema version; add `schema_base_version` to make it conditional; see [Schema evolution](schemas.md#schema-evolution) |
...
A topic can carry a JSON Schema, set at create time or with `PATCH`, and every produce is then validated at the broker before it touches disk. Versions are append-only with a fail-closed compatibility check, fan-out children inherit the parent's history, and `GET /v1/topics/{name}/schema` returns every version. The whole story, including the exact validation rules and what each keyword may change between versions, is on its own page: [Schemas](schemas.md).
