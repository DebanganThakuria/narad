...
The `PATCH` answers once the new version is committed and applied on the leader **and on the node that took your request**, so a read or a produce you send to that same node right after the response already sees the new version. Every other node applies it within replication latency (milliseconds) and validates the next produce it receives against it; the registry on each node is keyed by the metastore's schema version, so no restart or cache expiry is involved. A produce that reaches one of those other nodes inside that window is validated against the previous version. Increasing partitions, and any other alteration, leaves the schema untouched. Deleting the topic deletes the history with it; a recreated topic of the same name starts with no schema, or at v1 with whatever it is created with.
