...
**A soak shaped like production.** In September 2026 we ran a workload modelling a payments company through seven topics at about 1,000 messages a second: two firehoses with fast handlers, an outbound-webhook topic whose handlers were slow, uneven, and sometimes gave up entirely, slow refunds, a five-minute settlement burst with 8 KB payloads, an audit topic drained periodically, and a config topic idle for minutes at a time. That mix was deliberate: it reaches out-of-order acks, stranded leases, cold partitions and retention expiry, none of which a uniform benchmark ever touches. The first stretch, from 12 September with three driver processes, was invalidated on the 14th when the cluster-RPC certificate expired and partitioned the data plane. The clean run that followed, 17 hours over 16 and 17 September with five processes at the same total rate, produced 44.8 million messages. Its undelivered-message counter stayed at zero; its redelivery-after-ack counter finished at 32, which has not been explained. The soak was retired on 17 September, harness included, so that figure cannot be re-derived from the current tree.
...
- **Weeks of track record, not years.** The longest soak we ran lasted days, not years. Kafka, RabbitMQ, NATS and Redis have all been through Jepsen; our evidence is our own chaos matrix, self-administered, and worth exactly that.
