...
**A soak shaped like production.** From 12 to 17 September 2026, five processes drove about 1,000 messages a second through seven topics modelling a payments company: two firehoses with fast handlers, an outbound-webhook topic whose handlers were slow, uneven, and sometimes gave up entirely, slow refunds, a five-minute settlement burst with 8 KB payloads, an audit topic drained periodically, and a config topic idle for minutes at a time. That mix was deliberate: it reaches out-of-order acks, stranded leases, cold partitions and retention expiry, none of which a uniform benchmark ever touches. Over 44.8 million messages the undelivered-message counter stayed at zero; the redelivery-after-ack counter finished at 32, which is not yet explained.
