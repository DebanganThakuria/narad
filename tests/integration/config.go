...
func parseConfig(args []string) (config, error) {
	...
	flagSet.StringVar(&cfg.mode, "mode", modeLoad, "driver mode: load, chaos, steady, xnode, edge, soak")
	...
	flagSet.DurationVar(&cfg.timeout, "timeout", 2*time.Minute, "overall driver timeout; 0 in soak mode runs until stopped")
	...
	flagSet.StringVar(&cfg.metricsAddr, "metrics-addr", "", "soak mode: serve Prometheus metrics on this address (e.g. :9300); empty disables it")
	flagSet.Float64Var(&cfg.rateScale, "rate-scale", 1.0, "soak mode: fraction of each profile's modelled rate this process carries, so N processes at 1/N reproduce the modelled company once")
	flagSet.StringVar(&cfg.soakProfiles, "soak-profiles", "", "soak mode: comma-separated profile names to run; empty runs them all")
	...
	// A soak is meant to run until something stops it, so it is the one
	// mode where no timeout is a legitimate answer.
	if cfg.timeout <= 0 && cfg.mode != modeSoak {
		return cfg, errors.New("--timeout must be > 0 (use --mode soak for an open-ended run)")
	}
